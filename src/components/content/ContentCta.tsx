...
export function ContentCta({
  ...
  return (
    <div
      className={`mt-8 rounded-sm border border-neutral-200 bg-neutral-50 px-5 py-7 max-md:px-5 max-md:py-8 md:px-6 md:py-8 ${className}`.trim()}
    >
      {title ? (
        <p className="max-w-[95%] text-lg font-medium tracking-tight text-neutral-950 md:max-w-none md:text-xl">
          {title}
        </p>
      ) : null}
      {subtitle ? (
        <p className="mt-2 max-w-full text-pretty text-base font-light leading-relaxed text-neutral-600 max-md:max-w-[95%] md:text-lg">
          {subtitle}
        </p>
      ) : null}
      <WhatsAppLink
